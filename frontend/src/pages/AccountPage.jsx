...
function TradeModalInline({ stock, mode, onSuccess, exRate = 1380 }) {
  ...
  const expectedAvgPriceKrw = newTotalQty > 0
    ? ((currentAvgPriceKrw * currentQty) + total) / newTotalQty
    : krwPrice;
  ...
  const setQtyCapped = (newQty) => {
    const max = mode === "buy" ? maxBuyQty : maxSellQty;
    setQuantity(Math.max(0, Math.min(newQty, max)));
  };
  ...
  return (
    <div className="smt-body">
      ...
      {holding && mode === "buy" && (
        <div className="smt-info-row dim">
          <span>현재 보유</span>
          <strong>{fmt(currentQty)}주 (평단 {fmt(Math.round(currentAvgPriceKrw))}원)</strong>
        </div>
      )}
      ...
      <div className="smt-input-group">
        ...
        <div className="smt-qty-row">
          ...
          <input type="number" value={quantity}
            onChange={e => setQtyCapped(Number(e.target.value) || 0)}
            min={0}
      ...
      <div className="smt-total">
        <span>총 {mode==="buy"?"매수":"매도"} 금액</span>
        <strong>{fmt(Math.round(total))}원</strong>
      </div>
