...
export default function StockDetailPage({ user }) {
  ...
  const fetchPrice = async () => {
    ...
    try {
      ...
      const price = dom ? await getDomesticPrice(symbol) : await getOverseasPrice(symbol, info.exchange || getExchangeCode(info.market));
      setStock({ ...info, ...price });
    } catch (e) {
  ...
  if (loading && !stock) return <div className="detail-page"><div className="detail-loading"><div className="loading-spinner"/></div></div>;
          ...
          <div className="detail-stock-header">
            ...
            <div className="detail-price-area">
              ...
              <span className={`detail-big-change ${isUp(stock.changePercent)?"up":"dn"}`}>
                {isUp(stock.changePercent)?"▲":"▼"} {stock.change?fmt(Math.abs(Number(String(stock.change).replace(/[+]/g,"")))):""} ({stock.changePercent?fmtChange(stock.changePercent):"-"})
              </span>
            </div>
          </div>
