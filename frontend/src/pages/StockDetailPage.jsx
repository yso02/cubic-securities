...
export default function StockDetailPage({ user }) {
  ...
  useEffect(() => {
    ...
    try {
      const info = saved
        ? JSON.parse(saved)
        : { symbol, name: symbol, market: "KOSPI" };
      const recent = JSON.parse(sessionStorage.getItem("cubic_recent") || "[]");
      const updated = [
        info,
        ...recent.filter((s) => s.symbol !== symbol),
      ].slice(0, 10);
  ...
  const fetchPrice = async () => {
    ...
    try {
      ...
      const price = dom
        ...
        : await getOverseasPrice(
            symbol,
            info.exchange || getExchangeCode(info.market),
          );
      setStock({ ...info, ...price });
    } catch (e) {
      ...
      onConnect: () => {
        ...
        if (dom) {
          ...
          client.subscribe(`/topic/domestic/${stock.symbol}`, (msg) => {
            try {
              setStock((prev) =>
                prev ? { ...prev, ...JSON.parse(msg.body) } : prev,
              );
            } catch {}
          });
        } else {
          ...
          <>
            ...
            <div className="detail-stock-header">
              ...
              <div className="detail-price-area">
                <span className="detail-big-price">
                  {fmtPrice(stock.price, stock.market)}
                </span>
                <span
                  className={`detail-big-change ${isUp(stock.changePercent) ? "up" : "dn"}`}
                >
                  {isUp(stock.changePercent) ? "▲" : "▼"}{" "}
                  {stock.change
                    ? fmt(
                        Math.abs(
                          Number(String(stock.change).replace(/[+]/g, "")),
                        ),
                      )
                    : ""}{" "}
            ...
            <div className="detail-actions">
              <button className="da-buy" onClick={() => handleTrade("buy")}>
                매수
              </button>
              <button className="da-sell" onClick={() => handleTrade("sell")}>
                매도
              </button>
            </div>
            ...
            <div className="detail-news">
              <div className="news-header">
                <span className="news-title">{stock.name} 관련 뉴스</span>
