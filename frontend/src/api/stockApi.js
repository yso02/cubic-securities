...
export const getBalance = async () => {
  const r = await api.get("/api/trade/balance");
  return r.data;
};
export const getHoldings = async () => {
  const r = await api.get("/api/trade/holdings");
  return r.data;
};
export const getOrders = async () => {
  const r = await api.get("/api/trade/orders");
  return r.data;
};
...
export const getOverseasDetail = async (symbol, exchange = "NAS") => {
  const res = await api.get(
    `/api/stocks/detail/overseas/${symbol}?exchange=${exchange}`,
  );
  return res.data;
};
...
export const getOverseasOrderbook = async (symbol, exchange = "NAS") => {
  const res = await api.get(
    `/api/stocks/orderbook/overseas/${symbol}?exchange=${exchange}`,
  );
  return res.data;
};
